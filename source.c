...
void render(SDL_Surface* screen, SDL_Surface* background, Button buttons[], int buttonCount, TTF_Font* font) {
    ...
    if (currentState == MAIN_MENU) {
        ...
        SDL_Color textColor = {255,255,255}; //  color for text
...
void render_timer(SDL_Surface* screen, TTF_Font* font) {
    ...
    // Afficher le sablier
    int frame = ((int)elapsed_time) % 4; // 0,1,2,3 répétés
    SDL_Surface* sablier = sablier_images[frame];
    if (sablier) {
        SDL_Rect pos;
        pos.x = SCREEN_W - sablier->w - 10; // Affiché en haut à droite
        pos.y = 10;
        SDL_BlitSurface(sablier, NULL, screen, &pos);
    }

    // Afficher le texte du timer
    char timer_text[50];
    sprintf(timer_text, "Time: %.0f s", elapsed_time);

    SDL_Color textColor = {255, 255, 255}; // White color
    SDL_Surface* timer_surface = TTF_RenderText_Solid(font, timer_text, textColor);

    if (timer_surface) {
        apply_surface(SCREEN_W - timer_surface->w - 10, sablier->h + 20, timer_surface, screen);
        SDL_FreeSurface(timer_surface);
    }
}

void load_sablier_images() {
    sablier_images[0] = load_image("assets/sablier1.png");
    sablier_images[1] = load_image("assets/sablier2.png");
    sablier_images[2] = load_image("assets/sablier3.png");
    sablier_images[3] = load_image("assets/sablier4.png");
}

void free_sablier_images() {
    for (int i = 0; i < 4; i++) {
        if (sablier_images[i]) {
            SDL_FreeSurface(sablier_images[i]);
            sablier_images[i] = NULL;
        }
    }
}
